...
(() => {
  ...
  window.RaceSuspension=class {
    ...
    apply(body,state){
      ...
      // Camera blending reduces perceived cockpit vibration to 40%.
      const roadRide=Math.sin(this.travel*2.8)*Math.min((state.speed||0)/180,1)*.003;
      body.position.y=state.mode==='menu'||state.mode==='countdown'?0:roadRide;
    }
  };
})();
