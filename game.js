(() => {
  ...
  let accumulator=0;
  function frame(t){const dt=clamp((t-last)/1000||0,0,.1);last=t;accumulator+=dt;while(accumulator>=1/120){update(1/120);accumulator-=1/120;}if(state.mode!=='menu')updateHUD();audio.update();scene.render(state,['paused','crashed'].includes(state.mode)?0:dt,t/1000);requestAnimationFrame(frame);}requestAnimationFrame(frame);
})();
